...
/* To be logical or'd with an internal register address when repeated reads are needed */
...
/**
 * @brief Reads the AS5013 joystick's x component. Note that its format is natively a signed two's complement
 * eight bit integer.
 */
HAL_StatusTypeDef AS5013_get_x(I2C_HandleTypeDef *hi2c, int8_t *read_result);

/**
 * @brief Reads the AS5013 joystick's y component. Note that its format is natively a signed two's complement
 * eight bit integer.
 */
